...
export default function Terminal({ startInHero = true, sessionId }) {
 ...
 const send = async (e) => {
  ...
  // ---------- HERO (centered input card like the reference) ----------
 // ---------- HERO (centered mascot + title + input) ----------
if (hero) {
  return (
    <div className="w-full">
      {/* Mascot + Title (centered) */}
    ...
    <span className="text-white">AGENT</span>
      ...
      {/* Input bar (centered, under the title) */}
...
>
  ...
  // ---------- CONVERSATION (list + bottom-docked input card) ----------
  return (
    <div className="relative w-full max-w-5xl">
      {/* messages feed */}
      <div className="pb-40">{/* space for the bottom input card */}
        {messages.map((m, i) => (
...
">
                ...
                <div className="px-4 pb-4">
                  <div className="mt-1 text-white/90 font-bold">
                    {/* first line as headline if exists */}
                    {m.text.split('\n')[0]}
                  </div>
                  {/* rest of message */}
                  {m.text
