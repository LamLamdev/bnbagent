...
const APP_VERSION = 'tutorai_v1';         // NEW: Version check
const LS_CHATS = 'tutorai_chats';         // CHANGED from 'bnb_agent_chats'
const LS_PREFIX = 'tutorai_chat_';        // CHANGED from 'bnb_agent_chat_'

const safeParse = (s, f) => { try { return JSON.parse(s) ?? f; } catch { return f; } };
const hasWin = () => typeof window !== 'undefined';

// NEW: Clear old BNBAgent data on first load
function clearOldData() {
  if (!hasWin()) return;
  
  const storedVersion = localStorage.getItem('app_version');
  if (storedVersion !== APP_VERSION) {
    // Clear all old BNBAgent data
    localStorage.removeItem('bnb_agent_chats');
    
    // Clear all individual chat messages
    const keys = Object.keys(localStorage);
    keys.forEach(key => {
      if (key.startsWith('bnb_agent_chat_')) {
